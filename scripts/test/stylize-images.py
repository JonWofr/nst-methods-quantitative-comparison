...
def main():
    parser = argparse.ArgumentParser(description='Script which generates 100 stylized-images corresponding to the 100 content-style-image pairs for a given method. Providing style- or content-reference will not trigger any NST. Instead the style- or content-image of the content-style-image pair will be copied but will be named just like it would have been if it would have been generated. This is useful to mock the E or C statistic and serves as a reference for the comparison.')
    parser.add_argument('--method-name', required=True, choices=['collaborative-distillation', 'fast-neural-style', 'pytorch-adain', 'pytorch-neural-style-transfer', 'pytorch-wct', 'content-reference', 'style-reference'], help='The name of the NST method for which the stylized-images should be generated.')
    parser.add_argument('--method-dir-url', required=True, help='The url to the local repository of the NST method. Can be relative or absolute. If the method-name style- or content-reference has been provided method-dir-url can simply be . .')
...
def parse_command(method_name, content_image_file_url, style_image_file_url, stylized_image_file_url, fast_neural_style_trained_models_dir_url):
    command = {
        ...
        'content-reference': f'cp {content_image_file_url} {stylized_image_file_url}',
        'style-reference': f'cp {style_image_file_url} {stylized_image_file_url}'
    }
